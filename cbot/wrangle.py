...
def decode_and_normalize_swap(
    ...
    amount0_raw = int.from_bytes(bytes.fromhex(amount0_hex), byteorder="big", signed=True)
    amount1_raw = int.from_bytes(bytes.fromhex(amount1_hex), byteorder="big", signed=True)
    sqrt_price_x96 = int.from_bytes(
        bytes.fromhex(sqrt_price_x96_hex), byteorder="big", signed=False
    )
...
weth_swaps_decoded_df = weth_swaps_df.with_columns(
